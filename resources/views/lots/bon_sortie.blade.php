...
<head>
    <meta charset="UTF-8">
    <title>Bon de Sortie Chambre - {{ $lot->numero }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="header">
        <div class="titre">🧊 CDPHM - Mahajanga</div>
        <div class="sous-titre">Centre de Distribution des Produits Halieutiques</div>
        <div class="bon-type">📋 BON DE SORTIE CHAMBRE FROIDE</div>
    </div>
    ...
    <table>
        ...
        <tr>
            ...
            <td class="value-col">{{ number_format($lot->poids_entree, 2) }} kg</td>
        </tr>
        <tr>
            <td class="label-col">⚖️ Contre-pesée (poids réel)</td>
            <td class="value-col"><strong>{{ number_format($lot->poids_sortie ?? $lot->poids_entree, 2) }} kg</strong></td>
        </tr>
        <tr>
            <td class="label-col">📆 Durée en chambre</td>
            <td class="value-col">{{ (int) $jours }} jour(s)</td>
        </tr>
        <tr>
            <td class="label-col">💰 Tarif appliqué</td>
            <td class="value-col">{{ number_format($tarifChambre, 0, ',', ' ') }} Ar/kg/jour</td>
        </tr>
    </table>
